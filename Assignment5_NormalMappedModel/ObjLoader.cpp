...
void ObjLoader::processMtllibLine(const QVector<std::string>& splitLine) {
	if (splitLine.size() != 2) {
		throw std::invalid_argument("Line does not contain a proper mtllib declaration.");
	}
...
void ObjLoader::processVertexPositionLine(const QVector<std::string>& splitLine) {
	if (splitLine.size() != 4) {
		throw std::invalid_argument("Line does not contain an entry for each of the three components of a position vector.");
	}
...
void ObjLoader::processVertexNormalLine(const QVector<std::string>& splitLine) {
	if (splitLine.size() != 4) {
		throw std::invalid_argument("Line does not contain an entry for each of the three components of a normal vector.");
	}
...
void ObjLoader::processTextureCoordinateLine(const QVector<std::string>& splitLine) {
	if (splitLine.size() != 3) {
		throw std::invalid_argument("Line does not contain an entry for each of the two components of a vertex texture vector.");
	}
...
QVector3D ObjLoader::processVertexIndices(const std::string& vertexIndices) {
	...
	if (splitIndices.size() != 3) {
		throw std::invalid_argument("Vertex index specification does not contain an entry for each of an associated vertex, vertex texture, and vertex normal index.");
	}
