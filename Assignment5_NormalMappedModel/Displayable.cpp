...
Displayable::Displayable() : vbo_(QOpenGLBuffer::VertexBuffer), ibo_(QOpenGLBuffer::IndexBuffer), diffuseMap_(QOpenGLTexture::Target2D), normalMap_(QOpenGLTexture::Target2D), numTris_(0), rotationAxis_(0.0, 1.0, 0.0), rotationSpeed_(0.125) {
	rotationAngle_ = 0.0;
}
...
void Displayable::init(TranslatedObj* object) {
...
void Displayable::update(const qint64 msSinceLastFrame) {
...
void Displayable::setModelMatrix(const QMatrix4x4& transform) {
	modelMatrix_ = transform;
}

void Displayable::setRotationAxis(const QVector3D& axis) {
	rotationAxis_ = axis;
}

void Displayable::setRotationSpeed(float speed) {
	rotationSpeed_ = speed;
}

Displayable* Displayable::createFromFile(const std::string& filePath) {
	Displayable* renderable = new Displayable();
